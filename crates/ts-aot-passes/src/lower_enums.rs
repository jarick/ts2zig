...
use ts_aot_ir_hir::{HirDecl, HirExpr, HirProgram, HirStmt};
...
fn rewrite_callee(callee: &mut ts_aot_ir_hir::HirCallee, map: &HashMap<(Atom, Atom), Atom>) {
    match callee {
        ts_aot_ir_hir::HirCallee::Function(_)
        | ts_aot_ir_hir::HirCallee::Closure(_)
        | ts_aot_ir_hir::HirCallee::Runtime { .. } => {}
        ts_aot_ir_hir::HirCallee::Indirect(expr) => rewrite_expr(expr, map),
    }
}
