...
pub(crate) fn dump_decl(decl: &MirDecl, d: &mut Dumper<'_>) {
    match decl {
...
fn dump_function(f: &MirFunctionDecl, d: &mut Dumper<'_>) {
    d.write(&format!(
        "fn #{} {}({}) -> {}",
        f.id.raw(),
        dump_sym(f.name, d),
...
fn dump_param(p: &MirParam, d: &Dumper<'_>) -> String {
    format!("{}: {}", dump_sym(p.name, d), p.ty.raw())
}

fn dump_struct(s: &MirStructDecl, d: &mut Dumper<'_>) {
...
fn dump_global(g: &MirGlobalDecl, d: &mut Dumper<'_>) {
    d.line(&format!(
        "global {}: {} (mut: {})",
        dump_sym(g.name, d),
        g.ty.raw(),
